...
All five exist: `MLambda.AI.Logic` (six samples), `MLambda.AI.Agent` (three agents and the theory
beneath them), `MLambda.AI.ML` (four models), `MLambda.AI.Learning` (a bandit, two cliff learners and the
algebra they rely on), and the capstone, `MLambda.AI.Actuarial` — three agents, one real house, one
